    ...
    <div class="seller-commission-card overflow-hidden">
        <div class="table-responsive">
            <table class="table seller-commission-table mb-0">
                <thead><tr><th>ردیف</th><th>شماره فاکتور</th><th>تاریخ ثبت اولیه پیش‌فاکتور</th><th>نام مشتری</th><th>مبلغ نهایی</th></tr></thead>
                <tbody>
                @foreach($document->items as $item)
                    <tr><td>{{ $loop->iteration }}</td><td class="fw-bold">{{ $item->invoice_number_snapshot }}</td><td>{{ App\Support\JalaliDate::date($item->invoice_date_snapshot) }}</td><td>{{ $item->customer_name_snapshot }}</td><td>{{ App\Support\Currency::formatRial($item->invoice_total_snapshot) }}</td></tr>
                @endforeach
                </tbody>
                <tfoot><tr class="fw-bold"><td colspan="4">جمع کل</td><td>{{ App\Support\Currency::formatRial($document->total_sales_amount) }}</td></tr></tfoot>
            </table>
        </div>
    </div>
</div>
