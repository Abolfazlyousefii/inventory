...
@section('content')
<style>
.invoice-edit-page{max-width:100%;overflow-x:hidden;background:#f8fbff;font-size:.88rem}.edit-shell{background:#fff;border:1px solid #dbeafe;border-radius:18px;box-shadow:0 10px 26px rgba(30,64,175,.06)}.edit-card{border:1px solid #dbeafe;border-radius:16px;box-shadow:0 8px 22px rgba(30,64,175,.05)}.edit-card .card-header{background:#eff6ff;border-bottom:1px solid #dbeafe;font-weight:900;color:#1e3a8a}.invoice-action-bar{position:sticky;top:0;z-index:20;background:rgba(248,251,255,.96);border:1px solid #dbeafe;border-radius:16px;padding:10px;backdrop-filter:blur(6px)}.invoice-action-bar .btn{border-radius:10px;font-weight:800}.info-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.75rem}.info-box{background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:.75rem;min-width:0}.info-label{font-size:.76rem;color:#64748b}.info-value{font-weight:800;overflow-wrap:anywhere}.table-responsive{overflow-x:auto}#invoiceItemsTable{min-width:760px}.modal{z-index:1060}.modal-backdrop{z-index:1055}.modal-content{border-radius:18px}@media(max-width: 768px){.info-grid{grid-template-columns:1fr 1fr}.invoice-action-bar .btn{flex:1 1 150px}}@media(max-width: 576px){.info-grid{grid-template-columns:1fr}#invoiceItemsTable{min-width:680px}}
</style>
<div class="container py-4 invoice-edit-page">
  <div class="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-3">
    <div><h3 class="mb-1">ویرایش فاکتور {{ $invoice->uuid }}</h3><div class="text-muted">مدیریت اقلام، ثبت پرداخت و یادداشت در یک صفحه</div></div>
  </div>
  ...
  <div class="invoice-action-bar mb-3 d-flex gap-2 flex-wrap align-items-center">
    <a class="btn btn-primary" href="#items-editor">افزودن / ویرایش اقلام</a>
    <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#paymentModal" @disabled(!$canRegisterPayments || $remainingAmount <= 0)>افزودن پرداخت</button>
    <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#noteModal">افزودن یادداشت</button>
    <a class="btn btn-outline-dark" href="{{ route('invoices.print', $invoice->uuid) }}" target="_blank">چاپ</a>
  ...
  <form method="POST" action="{{ route('invoices.update', $invoice->uuid) }}" class="card edit-card mb-3" id="items-editor">
    @csrf @method('PUT')
    <div class="card-header d-flex justify-content-between align-items-center gap-2 flex-wrap"><span>اقلام فاکتور</span><button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#addInvoiceItemModal" @disabled(!$canEditItemsWithCollectionFlow)>+ افزودن کالا</button></div>
    <div class="card-body">
    ...
    </div><div class="card-footer text-end"><button class="btn btn-success" @disabled(!$canEditItemsWithCollectionFlow)>ذخیره تغییرات اقلام</button></div>
  </form>

  <div class="card edit-card"><div class="card-header">یادداشت‌های قبلی</div><div class="card-body">@forelse($invoice->notes as $note)<div class="border rounded p-2 mb-2 bg-white">{{ $note->body ?? $note->note }}<div class="small text-muted">{{ $note->user?->name ?? '—' }} | {{ $note->created_at ? Jalalian::fromDateTime($note->created_at)->format('Y/m/d H:i') : '—' }}</div></div>@empty<div class="text-muted">یادداشتی ثبت نشده است.</div>@endforelse</div></div>
</div>
