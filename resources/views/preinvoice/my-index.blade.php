    ...
    @forelse($orders as $order)
      ...
      <article class="my-sales-document {{ $isNeedsAction ? 'needs-action' : '' }}" data-document-card>
        <div class="my-sales-document__summary">
          ...
          <div class="my-sales-document__status"><span class="my-sales-document__label">وضعیت</span><span class="badge {{ $statusBadge($summary) }}">{{ $summary['status_label'] }}</span>@if($isNeedsAction)<small class="my-sales-document__notice">{{ $summary['needs_action_label'] }}</small>@endif</div>
          ...
          <div class="my-sales-document__actions"><a href="{{ $summary['view_url'] }}" class="btn btn-sm btn-outline-primary" data-document-action>{{ $summary['has_invoice'] ? 'مشاهده فاکتور' : 'مشاهده' }}</a><button type="button" class="btn btn-sm btn-outline-secondary" data-document-toggle aria-expanded="false" aria-controls="{{ $documentDomId }}"><span class="document-toggle-text">جزئیات</span></button></div>
        </div>
