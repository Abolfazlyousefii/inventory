...
<form method="POST" action="{{ route('product-deactivation-documents.store') }}" class="card">
    ...
    <div class="card-body">
        ...
        <div id="selectedProduct" class="border rounded p-3 my-3 {{ $selectedProduct ? '' : 'd-none' }}">
            ...
            <div class="mt-2">وضعیت فعلی: <span id="statusBadge" class="badge {{ $selectedProduct?->is_sellable ? 'text-bg-success' : 'text-bg-secondary' }}">{{ $selectedProduct?->is_sellable ? 'فعال' : 'غیرفعال' }}</span> <span id="variantSummary" class="small text-muted">@if($selectedProduct){{ $selectedProduct->sellable_variants_count }} از {{ $selectedProduct->structural_variants_count }} تنوع قابل فروش@endif</span></div>
        </div>
...
@php
    $selectedPayload = $selectedProduct ? ['id' => $selectedProduct->id, 'name' => $selectedProduct->name, 'code' => $selectedProduct->code ?: $selectedProduct->sku, 'category' => $selectedProduct->category?->name, 'is_sellable' => (bool) $selectedProduct->is_sellable, 'structural_variants_count' => (int) $selectedProduct->structural_variants_count, 'sellable_variants_count' => (int) $selectedProduct->sellable_variants_count] : null;
@endphp
<script>
...
async function selectProduct(product){current=product;$('productId').value=product.id;$('selectedName').textContent=product.name;$('selectedMeta').textContent=`کد: ${product.code||'—'} | دسته‌بندی: ${product.category||'—'}`;$('statusBadge').textContent=product.is_sellable?'فعال':'غیرفعال';$('statusBadge').className='badge '+(product.is_sellable?'text-bg-success':'text-bg-secondary');$('variantSummary').textContent=`${product.sellable_variants_count} از ${product.structural_variants_count} تنوع قابل فروش`;$('selectedProduct').classList.remove('d-none');$('operationFields').classList.remove('d-none');$('submitButton').disabled=false;$('searchResults').innerHTML='';const preferred=product.is_sellable?'deactivate':'activate';document.querySelector(`[name=action_type][value=${preferred}]`).checked=true;renderReasons();await loadVariants()}
