...
class ProductPriceListPdfTest extends TestCase
{
    ...
    public function test_pdf_has_download_filename(): void { $this->signIn(); $this->product('گارد'); $header=$this->get(route('admin.product-exports.download'))->headers->get('Content-Disposition'); $this->assertStringContainsString('attachment', $header); $this->assertStringContainsString('aria-gostar-price-list-', $header); }
    ...
    private function product(string $name, ?int $price=1000, ?string $image=null, ?Category $category=null): Product { $category ??= Category::firstOrCreate(['name'=>'Price list test']); return Product::create(['name'=>$name,'sku'=>'PDF-'.uniqid(),'price'=>$price ?? 0,'stock'=>1,'image_path'=>$image,'category_id'=>$category->id,'is_sellable'=>true]); }
    private function variant(Product $product,string $model,string $color,?int $price=1890000,?ModelList $modelList=null): ProductVariant { $modelList ??= ModelList::create(['brand'=>'Apple','model_name'=>$model]); $colorModel=Color::create(['name'=>$color,'code'=>'C-'.uniqid()]); return ProductVariant::create(['product_id'=>$product->id,'model_list_id'=>$modelList->id,'color_id'=>$colorModel->id,'variant_name'=>$model,'variety_name'=>$color,'sell_price'=>$price ?? 0,'stock'=>3,'is_active'=>true,'sales_enabled'=>true]); }
}
