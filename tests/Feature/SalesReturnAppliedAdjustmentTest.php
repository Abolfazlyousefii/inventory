...
class SalesReturnAppliedAdjustmentTest extends TestCase
{
    ...
    public function test_applied_edit_reverses_old_stock_and_queues_reapproval(): void { $s=file_get_contents(app_path('Services/SalesReturnAppliedAdjustmentService.php')); $this->assertStringContainsString('reverseInventory', $s); $this->assertStringContainsString('queueSalesReturn', $s); $this->assertStringContainsString('STATUS_PENDING_WAREHOUSE', $s); }
    public function test_applied_edit_reverses_old_credit_and_defers_new_credit(): void { $s=file_get_contents(app_path('Services/SalesReturnAppliedAdjustmentService.php')); $this->assertStringContainsString('reverseLedger', $s); $this->assertStringNotContainsString('$this->creditLedger(', $s); }
