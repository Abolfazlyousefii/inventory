...
it('submits 220 products through a complete json payload without products 198 validation loss', function () {
    ...
    expect($order->status)->toBe(PreinvoiceOrder::STATUS_PENDING_FINANCE)
        ->and($order->items()->count())->toBe(220)
        ->and((int) $order->items()->sum('quantity'))->toBe(440)
        ->and((int) $order->total_price)->toBe($gross)
...
it('updates a large draft without deleting tail items and allows intentional deletion only', function () {
    $user = largePayloadUser();
    $rows = largePayloadProducts(220);
    $order = largePayloadOrder($user, $rows);
    $rows = $order->items->map(fn ($item) => [
        'item_id' => $item->id,
        'id' => $item->product_id,
        'product_id' => $item->product_id,
        'variety_id' => $item->variant_id,
    ...
    $beforeIds = $order->items->pluck('id')->all();
    ...
    expect($order->items)->toHaveCount(220)
        ->and($order->items->pluck('id')->all())->toBe($beforeIds)
        ->and((int) $order->total_price)->toBe($beforeTotal);

    $deleted = array_pop($rows);
    $this->actingAs($user)->put(route('preinvoice.draft.update', $order->uuid), largePayloadPost($rows, ['intent' => 'draft']))->assertSessionHasNoErrors();
    $order->refresh()->load('items');
    expect($order->items)->toHaveCount(219)
        ->and($order->items->pluck('id')->contains($deleted['item_id']))->toBeFalse();
});
...
it('rejects incomplete json payloads before mutating an existing draft', function (string $case) {
    ...
    $beforeItems = $order->items()->count();
    $beforeReservations = PreinvoiceDraftReservation::query()->where('preinvoice_order_id', $order->id)->count();

    $override = match ($case) {
        'count mismatch' => ['products_payload' => json_encode(array_slice($rows, 0, 219), JSON_THROW_ON_ERROR), 'products_payload_count' => 220],
        'broken json' => ['products_payload' => '{bad', 'products_payload_count' => 220],
        'not complete' => ['products_payload_complete' => 0],
        'empty payload' => ['products_payload' => ''],
        'missing id' => ['products_payload' => json_encode([['variety_id' => 1, 'quantity' => 1, 'price' => 1]], JSON_THROW_ON_ERROR), 'products_payload_count' => 1],
    ...
    $response = $this->actingAs($user)
        ->from(route('preinvoice.draft.edit', $order->uuid))
        ->put(route('preinvoice.draft.update', $order->uuid), largePayloadPost($rows, $override));

    $response->assertSessionHasErrors();
    $order->refresh();
    expect($order->items()->count())->toBe($beforeItems)
        ->and((int) $order->total_price)->toBe($beforeTotal)
        ->and($order->status)->toBe($beforeStatus)
...
it('renders the json payload contract instead of relying on legacy product inputs during submit', function () {
    ...
    expect($view)->toContain('name="products_payload"')
        ->and($view)->toContain('name="products_payload_count"')
        ->and($view)->toContain('name="products_payload_complete"')
        ->and($view)->toContain('collectProductsForSubmit')
        ->and($view)->toContain('prepareProductsPayloadForSubmit')
        ->and($view)->toContain('JSON.stringify')
        ->and($view)->toContain('disable legacy product inputs');
});
