...
class SalesReturnController extends Controller
{
    ...
    {
        $filters = $request->filters() + ['status' => $request->input('status', 'all')];
        $documents = $this->reports->buildDocumentQuery($filters)->with(['items.destinationWarehouse:id,name,type'])->paginate((int) ($filters['per_page'] ?? 30))->withQueryString();
    ...
    {
        return $this->pdfResponse('sales-returns.report-pdf', $this->reportData($request), 'sales-returns-report-'.now()->format('Ymd-His').'.pdf', 'landscape');
    }

    public function printReport(SalesReturnIndexRequest $request)
    {
        return view('sales-returns.report-print', $this->reportData($request));
    }
    ...
    public function show(SalesReturnDocument $document, SalesDocumentAccessService $accessService)
    {
        ...
        return view('sales-returns.show', compact('document', 'healthCheck', 'canViewSourceInvoice'));
    }
    ...
    public function edit(SalesReturnDocument $document)
    {
        ...
        return view('sales-returns.create', $this->formData() + ['document' => $document]);
    }
    ...
    public function cancel(Request $request, SalesReturnDocument $document)
    {
        ...
        return redirect()->route('sales-returns.show', $doc)->with('success', 'پیش‌نویس لغو شد.');
    }

    public function print(SalesReturnDocument $document)
    {
        $document->load('customer', 'invoice', 'items.destinationWarehouse', 'creator', 'applier');
    ...
    public function pdf(SalesReturnDocument $document)
    {
        ...
        return $this->pdfResponse('sales-returns.document-pdf', compact('document'), 'sales-return-'.$document->document_number.'.pdf');
    }
    ...
    private function reportData(SalesReturnIndexRequest $request): array
    {
        ...
        return ['filters' => $filters, 'documents' => $this->reports->buildDocumentQuery($filters)->with('items.destinationWarehouse:id,name,type')->limit(1000)->get(), 'summary' => $this->reports->summary($filters), 'reportService' => $this->reports];
    }
    ...
    {
        return ['warehouses' => Warehouse::where('is_active', true)->whereIn('type', ['central', 'return'])->get(), 'categories' => Category::orderBy('name')->get(), 'commissionEffectLabels' => SalesReturnDocument::commissionEffectLabels(), 'legacyReturnUrl' => route('vouchers.section.index', 'return-from-sale')];
    }
