...
use App\Services\{SalesDocumentAccessService,SalesReturnAppliedAdjustmentService,SalesReturnReportService,SalesReturnService};
...
class VoucherSalesReturnController extends Controller
{
    ...
    public function show(SalesReturnDocument $document, SalesDocumentAccessService $accessService){ $document->load('customer','invoice','items.destinationWarehouse','items.product','items.variant','creator','applier'); $healthCheck=$this->reports->appliedHealthCheck($document); $user=auth()->user(); $canViewSourceInvoice=$document->invoice && $user && PageAccessCatalog::userCanRoute($user,'vouchers.sales.show') && $accessService->canViewInvoiceReadonly($document->invoice,$user); return view('vouchers.return-from-sale.show',compact('document','healthCheck','canViewSourceInvoice')); }
