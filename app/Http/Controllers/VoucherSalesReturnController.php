...
class VoucherSalesReturnController extends Controller
{
 ...
 public function index(SalesReturnIndexRequest $request){ $filters=$request->filters()+['status'=>$request->input('status','all')]; $documents=$this->reports->buildDocumentQuery($filters)->with(['items.destinationWarehouse:id,name,type'])->paginate((int)($filters['per_page']??30))->withQueryString(); $legacyReturns=WarehouseTransfer::query()->where('voucher_type',WarehouseTransfer::TYPE_CUSTOMER_RETURN)->latest()->paginate(15,['*'],'legacy_page')->withQueryString(); return view('vouchers.return-from-sale.index',['documents'=>$documents,'filters'=>$filters,'summary'=>$this->reports->summary($filters),'tabCounts'=>$this->reports->tabCounts($filters),'warehouses'=>Warehouse::query()->whereIn('type',['central','return'])->orderBy('name')->get(['id','name','type']),'customers'=>Customer::query()->when($filters['customer_id']??null,fn($q,$id)=>$q->whereKey($id))->get(['id','first_name','last_name','mobile']),'products'=>Product::query()->when($filters['product_id']??null,fn($q,$id)=>$q->whereKey($id))->get(['id','name']),'variants'=>ProductVariant::query()->when($filters['product_variant_id']??null,fn($q,$id)=>$q->whereKey($id))->get(['id','variant_name','variant_code']),'users'=>User::query()->whereIn('id',array_filter([$filters['created_by']??null,$filters['applied_by']??null]))->get(['id','name']),'reportService'=>$this->reports,'legacyReturns'=>$legacyReturns]); }
