...
class VoucherSalesReturnController extends Controller
{
 ...
 public function updateApplied(UpdateAppliedSalesReturnRequest $request, SalesReturnDocument $document){ $data=$request->validated(); $doc=$this->adjustments->updateApplied($document,$this->enrich($request)+$data,auth()->id(),$data['adjustment_reason']); return redirect()->route('vouchers.return-from-sale.show',$doc)->with('success','اصلاحات سند ثبت نهایی ذخیره شد.'); }
