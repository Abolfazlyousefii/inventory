...
class ProductController extends Controller {
    ...
    public function data( Request $request, ProductSearchService $searchService ) {
        ...
        return response()->json([
            'data' => $products->getCollection()
                ->map(fn( $product ) => $searchService->productPayload($product))
                ->values(),
            'meta' => [
    ...
    public function variants( Product $product, Request $request, ProductSearchService $searchService ) {
        ...
        return response()->json([
            ...
            'data'    => $variants->getCollection()
                ->map(fn( $variant ) => $searchService->variantPayload($variant))
                ->values(),
            'meta'    => [
    ...
    public function warehouseStock( Product $product, Request $request ) {
        ...
        $allVariantIds      = $allVariants->pluck('id')
            ->map(fn( $id ) => (int) $id)
            ->values();
        ...
        $pagedVariants = $filteredVariants->slice($cursor, $limit)
            ->values();
        $rows          = $this->warehouseStockRows($product, $pagedVariants->pluck('id')
            ->map(fn( $id ) => (int) $id), $reservedTotalByVariant, $centralWarehouseId, true);
        ...
        return response()->json([
            ...
            'title'           => $selectedVariant ? $product->name . ' / ' . $this->variantDisplayTitle($selectedVariant) : $product->name,
    ...
    private function warehouseStockRows( Product $product, $variantIds, $reservedTotalByVariant, int $centralWarehouseId, bool $hasVariants ) {
        ...
        if ( !$hasVariants ) {
            return $stockRows->map(function ( $stock ) use ( $warehouses ) {
                ...
                ->filter(fn( $row ) => (int) $row['available_quantity'] > 0)
                ->values();
        }

        $stocksByVariantWarehouse = $stockRows->groupBy(fn( $stock ) => ( (int) $stock->product_variant_id ) . ':' . ( (int) $stock->warehouse_id ));
        ...
        foreach ( $variantIds as $variantId ) {
            ...
            foreach ( $warehouses as $warehouse ) {
                ...
                $availableInWarehouse = (int) ( $stocksByVariantWarehouse->get($key, collect())
                    ->sum('quantity') );
    ...
    public function store( Request $request ) {
        ...
        DB::transaction(function () use ( $data, $useModels, $useDesigns, $designNotes, $isSellable, $imagePath ) {
            ...
            $product = Product::create([
                ...
                'models'        => app(ProductVariantStructureService::class)->metadata($useModels, $data['model_list_ids'] ?? [], $useDesigns, $data['design_count'] ?? null, $designNotes->all()),
            ]);
        ...
        return redirect()
            ...
            ->with('success', 'کالا و تنوع‌ها با موفقیت ساخته شدند.');
    }
    ...
    public function update( Request $request, Product $product ) {
        ...
        DB::transaction(function () use ( $data, $request, $product, $newImagePath, $removeImage, &$oldImagePath ) {
            ...
            foreach ( ( $data['variants'] ?? [] ) as $v ) {
                ...
                $sellPrice = array_key_exists('sell_price', $v) && $v['sell_price'] !== '' ? (int) $v['sell_price'] : (int) ( $variant?->sell_price ?? 0 );

                $buyPrice = array_key_exists('buy_price', $v) && $v['buy_price'] !== '' ? (int) $v['buy_price'] : $variant?->buy_price;
                ...
                if ( $variant && array_key_exists('stock', $v) && $v['stock'] !== '' ) {
                    WarehouseStockService::set($centralWarehouseId, (int) $product->id, (int) $variant->id, (int) $v['stock']);
                }
            }

            $defaultDesignService = app(DefaultProductDesignService::class);
            $keepIds              = array_values(array_unique(array_merge($keepIds, $defaultDesignService->electricDefaultColorVariantIds($product))));
        ...
        return redirect()
            ->to($this->safeProductsReturnUrl($request->input('return_to')))
            ->with('success', 'کالا بروزرسانی شد.');
    }
    ...
    private function syncProductVariants( Product $product, array $data ): void {
        ...
        $modelIds = $useModels ? collect($data['model_list_ids'] ?? [])
            ->map(fn( $id ) => (int) $id)
            ->filter()
            ->unique()
            ->values() : collect([ null ]);
        ...
        $modelsById = $useModels ? ModelList::query()
        ...
        $defaultSellPrice = array_key_exists('sell_price', $data) && $data['sell_price'] !== null ? (int) $data['sell_price'] : 0;
        $defaultBuyPrice  = array_key_exists('buy_price', $data) && $data['buy_price'] !== null ? (int) $data['buy_price'] : null;
        ...
        foreach ( $modelIds as $modelId ) {
            ...
            foreach ( $designIndexes as $designIndex ) {
                ...
                $variantName = trim(collect([
                    ...
                    ->filter(fn( $part ) => trim((string) $part) !== '' && $part !== '—')
                    ->implode(' '));
    ...
    private function sanitizeUpdateVariants( Request $request, Product $product ): void {
        ...
        $productVariantIds = $product->variants()
            ->pluck('id')
            ->map(fn( $id ) => (int) $id)
            ->all();
    ...
    public function syncCrm( CrmProductSyncService $service ) {
        ...
        return redirect()
            ->route('products.index')
            ->with('success', "همگام‌سازی انجام شد. ایجاد: {$res['created']} | بروزرسانی: {$res['updated']} | خطا: {$res['failed']}");
    }
