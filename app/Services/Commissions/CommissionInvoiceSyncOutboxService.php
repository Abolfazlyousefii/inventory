...
class CommissionInvoiceSyncOutboxService
{
    ...
    /**
     * Persist work in the same source transaction and deliver after commit.
     * The durable row makes post-commit PHP/process failure recoverable.
     */
    public function stage(
        ...
        // Safe deployment fallback if code becomes visible a few seconds before
        // the new migration is applied. We preserve old dirty-marker behavior.
        if (! Schema::hasTable('commission_invoice_sync_outboxes')) {
            $this->dirtyMarker->markDate($oldDate);
            $this->dirtyMarker->markDate($newDate);
        ...
        DB::afterCommit(function () use ($invoiceId): void {
            try {
                ...
                // Failure metadata + dirty fallback are already recorded by
                // processInvoice(). Reporting must not roll back source data.
                report($exception);
            }
        });
    }
    ...
    public function processInvoice(int $invoiceId): bool
    {
        ...
        try {
            return DB::transaction(function () use ($invoiceId): bool {
                ...
                // Merge multiple Invoice + InvoiceItem callbacks into one sync.
                ...
                CommissionInvoiceSyncOutbox::query()
