...
class CommissionInvoiceSyncService
{
    ...
    public function syncInvoice(
        ...
        $periodIds = $this->affectedPeriodIds($invoiceId, $invoiceNumberSnapshot, [$oldDate, $newDate]);
        ...
        return DB::transaction(function () use ($invoiceId, $invoiceNumberSnapshot, $periodIds) {
            ...
            foreach ($periods as $period) {
                ...
                $existingForInvoice = $this->activeInvoiceEntries($period, $invoiceId, $invoiceNumber);
                ...
                foreach ($invoice->items as $item) {
                    ...
                    $calculation = $this->calculator->calculate($invoice, $item, (int) $sellerId, $invoiceDate);
                    ...
                    // Lock by invoice item rather than invoice_id so an item moved
                    // between invoices cannot temporarily create two active rows.
                    ...
                    if ($attributes['missing_rate']) {
                        $this->warning($period, $invoice, (int) $item->id, 'missing_rate', 'ردیف فاکتور فاقد نرخ پورسانت است.');
                        $result['warnings']++;
                    }
                ...
                $stale = $this->activeInvoiceEntries($period, $invoiceId, $invoiceNumber)
                    ->filter(fn (CommissionLedgerEntry $entry) => $entry->invoice_item_id === null
    ...
    private function affectedPeriodIds(int $invoiceId, ?string $invoiceNumberSnapshot, array $dates): array
    {
        $ids = collect();
        ...
        foreach ($dates as $date) {
            ...
            $ids = $ids->merge(CommissionPeriod::query()
                ->whereIn('status', [CommissionPeriod::STATUS_OPEN, CommissionPeriod::STATUS_REVIEW])
                ->where('start_at', '<=', $date)
                ->where('end_at', '>', $date)
                ->pluck('id'));
        ...
        $ledgerIds = CommissionLedgerEntry::query()
            ...
            ->whereHas('period', fn ($query) => $query->whereIn('status', [CommissionPeriod::STATUS_OPEN, CommissionPeriod::STATUS_REVIEW]))
            ->pluck('commission_period_id');

        return $ids->merge($ledgerIds)->map(fn ($id) => (int) $id)->unique()->sort()->values()->all();
    }

    private function activeInvoiceEntries(CommissionPeriod $period, int $invoiceId, ?string $invoiceNumberSnapshot): Collection
    {
    ...
    private function supersedeEntries(Collection $entries): int
    {
        ...
        foreach ($entries as $entry) {
            ...
            $entry->update(['status' => CommissionLedgerEntry::STATUS_SUPERSEDED, 'active_marker' => null]);
            $count++;
        }
        return $count;
    }
