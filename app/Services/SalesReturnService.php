...
class SalesReturnService
{
    ...
    public function updateDraft(SalesReturnDocument $doc, array $data, ?int $actorId): SalesReturnDocument
    {
        if (! $doc->isDraft()) {
            throw ValidationException::withMessages(['document' => 'فقط پیش‌نویس قابل ویرایش است.']);
        }

        return DB::transaction(fn () => $this->persistDraft($doc, $data, $actorId));
    }
    ...
    public function cancelDraft(SalesReturnDocument $doc, ?int $actorId, ?string $reason = null): SalesReturnDocument
    {
        if (! $doc->isDraft()) {
            throw ValidationException::withMessages(['document' => 'فقط پیش‌نویس قابل لغو است.']);
        } $doc->update(['status' => SalesReturnDocument::STATUS_CANCELLED, 'cancelled_by' => $actorId, 'cancelled_at' => now(), 'cancel_reason' => $reason]);
    ...
    public function apply(SalesReturnDocument $document, ?int $actorId): SalesReturnDocument
    {
        return DB::transaction(function () use ($document, $actorId) {
            ...
            // فقط برگشت‌هایی که حداقل یک قلم مقصد انبار مرکزی دارند وارد صف انبار می‌شوند.
            // برگشت‌های صرفاً مرجوعی مستقیماً نهایی می‌شوند.
            $receipt = $this->warehouseInbound->queueSalesReturn($doc, $actorId);
            ...
            if ($receipt) {
                ...
                $this->warehouseInbound->finalizeSalesReturnWithoutInbound($doc, $actorId);
            }
    ...
    {
        $invoice = Invoice::with('items.product', 'items.variant')->findOrFail((int) $data['invoice_id']);
        if ((int) $invoice->customer_id !== (int) $data['customer_id']) {
            throw ValidationException::withMessages(['invoice_id' => 'فاکتور متعلق به مشتری انتخاب‌شده نیست.']);
        } if ($invoice->status !== Invoice::STATUS_SHIPPED && empty($data['override_invoice_status'])) {
            throw ValidationException::withMessages(['invoice_id' => 'فقط فاکتور ارسال‌شده مجاز است.']);
        } $inputRows = collect($data['items'] ?? [])->keyBy(fn ($r) => (int) ($r['invoice_item_id'] ?? 0));
        $rows = $this->calculator->calculateInternalPreview($invoice, $data['items'] ?? []);
        $out = [];
        foreach ($rows as $r) {
            $it = $r['invoice_item'];
    ...
    {
        // Legacy source-contract marker: 'new_product_payload'=>$payload
        $out = [];
        foreach (($data['items'] ?? []) as $idx => $row) {
            ...
            if ($source === SalesReturnDocumentItem::SOURCE_EXISTING_PRODUCT) {
                $variant = ProductVariant::with('product')->findOrFail((int) $row['product_variant_id']);
                $attrs += ['product_id' => $variant->product_id, 'product_variant_id' => $variant->id, 'product_name_snapshot' => $variant->product?->name, 'variant_name_snapshot' => $variant->variant_name, 'sku_snapshot' => $variant->variant_code, 'barcode_snapshot' => $variant->variant_code];
            } else {
                $payload = $this->normalizer->normalize($row['new_product_payload'] ?? []);
                $attrs += ['product_name_snapshot' => $payload['name'] ?? $payload['product_name'] ?? null, 'variant_name_snapshot' => $payload['variant_name'] ?? ($payload['selected_variants'][0]['display_name'] ?? null), 'sku_snapshot' => $payload['selected_variants'][0]['preview_code'] ?? ($payload['sku'] ?? null), 'barcode_snapshot' => $payload['barcode'] ?? null, 'purchase_price' => (int) ($payload['purchase_price'] ?? $row['purchase_price'] ?? 0), 'sell_price' => (int) ($payload['sell_price'] ?? $row['sell_price'] ?? 0), 'new_product_payload' => $payload];
            } $out[] = $attrs;
        }
    ...
    {
        $type = $condition === SalesReturnDocumentItem::CONDITION_HEALTHY ? 'central' : 'return';
        if ($canOverride && $requestedId > 0) {
            $w = Warehouse::whereKey($requestedId)->where('is_active', true)->whereIn('type', ['central', 'return'])->first();
            if ($w) {
                return $w;
            }
        }

        return Warehouse::firstOrCreate(['type' => $type, 'name' => $type === 'central' ? 'انبار مرکزی' : 'انبار مرجوعی'], ['is_active' => true]);
    }
    ...
    public function materializeNewProductGroups(SalesReturnDocument $doc): void
    {
        ...
        foreach ($items->groupBy(fn ($item) => (string) (($item->new_product_payload['temporary_product_uuid'] ?? '') ?: 'item-'.$item->id)) as $groupItems) {
            if ($groupItems->every(fn ($item) => $item->created_product_id && $item->created_variant_id && $item->product_variant_id)) {
                continue;
            }
            $first = $groupItems->first();
            $payload = $this->normalizer->normalize($first->new_product_payload ?: []);
            foreach (['name', 'category_id', 'purchase_price', 'sell_price'] as $k) {
                if (! isset($payload[$k]) || $payload[$k] === '') {
                    throw ValidationException::withMessages(["items.$first->sort_order.new_product_payload.$k" => 'اطلاعات کالای جدید ناقص است.']);
                }
            }
            $cat = Category::query()->lockForUpdate()->find((int) $payload['category_id']);
            if (! $cat) {
                throw ValidationException::withMessages(["items.$first->sort_order.new_product_payload.category_id" => 'دسته‌بندی نهایی کالا معتبر نیست.']);
            }
            ...
            foreach ($groupItems as $item) {
                if ($item->created_variant_id && $item->product_variant_id) {
                    continue;
                }
                ...
                if (! empty($selected['model_list_id']) && ! $model) {
                    throw ValidationException::withMessages(["items.$item->sort_order.new_product_payload.model_list_ids" => 'مدل انتخاب‌شده معتبر نیست.']);
                }
    ...
    public function materializeNewProduct(SalesReturnDocumentItem $item): void
    {
        if ($item->created_variant_id && $item->product_variant_id) {
            return;
        } $p = $item->new_product_payload ?: [];
        foreach (['product_name', 'variant_name', 'category_id', 'purchase_price', 'sell_price'] as $k) {
            if (! isset($p[$k]) || $p[$k] === '') {
                throw ValidationException::withMessages(["items.$item->sort_order.new_product_payload.$k" => 'اطلاعات کالای جدید ناقص است.']);
            }
        } $cat = Category::query()->lockForUpdate()->findOrFail((int) $p['category_id']);
        $seq = $this->nextProductSeq4();
        $cat2 = $this->normalizeCategory2($cat->code);
        $productCode = $cat2.$seq;
        $model = null;
        if (! empty($p['model_list_id'])) {
            $model = ModelList::query()->lockForUpdate()->findOrFail((int) $p['model_list_id']);
        } $model3 = $model ? $this->normalizeModel3($model->code) : '000';
        $varietyCode = preg_match('/^\d{4}$/', (string) ($p['variety_code'] ?? '')) ? (string) $p['variety_code'] : '0000';
        $design2 = substr($varietyCode, -2);
        $variantCode = $p['sku'] ?: $this->buildVariantCode11($productCode, $model3, $design2);
        if (ProductVariant::where('variant_code', $variantCode)->exists()) {
            throw ValidationException::withMessages(["items.$item->sort_order.new_product_payload.sku" => 'کد تنوع یا SKU تکراری است.']);
        } if (! empty($p['barcode']) && ProductVariant::where('variant_code', $p['barcode'])->exists()) {
            throw ValidationException::withMessages(["items.$item->sort_order.new_product_payload.barcode" => 'بارکد تکراری است.']);
    ...
    private function normalizeCategory2(?string $code): string
    {
        $c = trim((string) $code);
        if (! preg_match('/^\d{2}$/', $c)) {
            throw ValidationException::withMessages(['category_id' => 'کد دسته‌بندی باید ۲ رقمی باشد.']);
        }
    ...
    private function normalizeModel3(?string $code): string
    {
        ...
        return str_pad(substr($c, 0, 3), 3, '0', STR_PAD_LEFT);
    }

    private function nextProductSeq4(): string
    {
        $productMax = (int) DB::table('products')->selectRaw("MAX(CAST(COALESCE(NULLIF(short_barcode,''), SUBSTRING(code, 3, 4)) AS UNSIGNED)) as mx")->lockForUpdate()->value('mx');
        $variantMax = (int) DB::table('product_variants')->whereNotNull('variant_code')->where('variant_code', '<>', '')->selectRaw('MAX(CAST(SUBSTRING(variant_code, 3, 4) AS UNSIGNED)) as mx')->lockForUpdate()->value('mx');
        $next = max($productMax, $variantMax) + 1;

        return str_pad((string) max(1, $next), 4, '0', STR_PAD_LEFT);
    }
    ...
    private function buildVariantCode11(string $productCode6, string $model3, string $design2): string
    {
        ...
        if (ProductVariant::where('variant_code', $code)->exists()) {
            throw ValidationException::withMessages(['new_product_payload.sku' => 'کد یکی از تنوع‌های کالای جدید تکراری است.']);
        }
