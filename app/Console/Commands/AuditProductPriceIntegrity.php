...
class AuditProductPriceIntegrity extends Command
{
    ...
    private function documentAnomalies(string $type): array
    {
        ...
        return DB::table($table.' as i')->leftJoin($doc.' as d', 'd.id', '=', 'i.'.($isInvoice ? 'invoice_id' : 'preinvoice_order_id'))
            ...
            ->map(fn ($i) => $this->emptyRow((int) $i->product_id, $i->variant_id ? (int) $i->variant_id : null) + ['updated_at' => $i->updated_at ?? null, 'anomaly_code' => $isInvoice ? 'A06' : 'A05', 'severity' => 'Critical', 'probable_root_cause' => ($isInvoice ? 'Invoice' : 'Preinvoice').' item has positive quantity and zero price.'])->all();
    }
